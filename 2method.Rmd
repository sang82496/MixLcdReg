...
binnedY = binningY(X, Y, B, min_count_ratio)
Y_bin = binnedY$Y_bin
...
for (t in 1:TT){
  time_index = time_indicator == t
  points(rep(t, sum(time_index)), Y_bin[time_index], cex = Y_count[time_index]/mean(Y_count))
}
```
...
```{r, send_to = "R/functions.R"}
#' initialization using GMR on binned data
#' 
#' @param binnedY the return of binningY function
<<K-param>>
#' @return flex
#' @return weight_init
#' @return idx_init
...
#' @return resi_init
#' @export
initial_GMR = function(binnedY, K){
  X = binnedY$X
  Y_bin = binnedY$Y_bin
  Y_count = binnedY$Y_count
  X_bin = binnedY$X_bin
  ...
  # initial clusters
  clust_init = flexmix::clusters(flex)
  weight_init = matrix(0, nrow = Nb, ncol = K)
  idx_init = matrix(F, nrow = Nb, ncol = K)
  for (i in 1:Nb){
    weight_init[i, clust_init[i]] = Y_count[i]
    idx_init[i, clust_init[i]] = T
  }
...
```{r, send_to = "R/functions.R"}
#' initialization using GMR on thresholded binned data
#' 
#' @param binnedY the return of binningY function
<<K-param>>
<<B-param>>
...
#' @return resi_init
#' @export
initial_GMR_thresholded = function(binnedY, K, B, min_count_ratio){
  X = binnedY$X
  Y = binnedY$Y
  Y_bin = binnedY$Y_bin
  Y_count = binnedY$Y_count
  ...
  # GMR intialization on thresholded binned data
  binnedY_init = binningY(X, Y, B, min_count_ratio)
  Y_bin_init = binnedY_init$Y_bin
  Y_count_init = binnedY_init$Y_count
  X_bin_init = binnedY_init$X_bin
  time_indicator_init = binnedY_init$time_indicator
  N_init = binnedY_init$N
  Nb_init = length(Y_count_init)
  
  
  # initialization
  flex = flexmix::flexmix(Y_bin_init ~ X_bin_init, k = K, weights = as.vector(Y_count_init))
  ...
  for (k in 1:K){
    theta0_init[[k]] = flexmix::parameters(flex, comp = k)[1]
    theta_init[[k]] = matrix(flexmix::parameters(flex, comp = k)[2:(p+1)])
    sigma_init[[k]] = flexmix::parameters(flex, comp = k)[p+2]
  }
  resi_init = calc_resi(X_bin, Y_bin, theta0_init, theta_init, K)
  ...
  for (i in 1:Nb){
    logLik = rep(0, K)
    for (k in 1:K){
      logLik[k] = dnorm(resi_init[[k]][i], sd = sigma_init[[k]])
    }
    clust_init[i] = which.max(logLik)
    weight_init[i, clust_init[i]] = Y_count[i]
    idx_init[i, clust_init[i]] = T
  }
...
```{r, send_to = "R/functions.R"}
#' initialization using GMR on non-binned data
#' 
#' @param binnedY the return of binningY function
<<K-param>>
#' @return flex
...
#' @return resi_init
#' @export
initial_GMR_nonBinned = function(binnedY, K){
  X = binnedY$X
  Y = binnedY$Y
  Y_bin = binnedY$Y_bin
  Y_count = binnedY$Y_count
  ...
  # GMR intialization on thresholded binned data
  binnedY_init = binningY(X, Y, B = 0, min_count_ratio = 0)
  Y_bin_init = binnedY_init$Y_bin
  Y_count_init = binnedY_init$Y_count
  X_bin_init = binnedY_init$X_bin
  time_indicator_init = binnedY_init$time_indicator
...
<<B-param>>
#' @param min_count_ratio min count ratio
#' @init_method init_method choice of the initialization method
<<lambda_alpha-param>>
<<lambda_theta-param>>
#' @return flex
#' @return weight_init
...
#' @return Q_every
#' @export
initialization = function(binnedY, K, B, min_count_ratio, init_method, lambda_alpha, lambda_theta){
  if (init_method == 'th') {
    initial = initial_GMR_thresholded(binnedY, K, B, min_count_ratio)
  } else if (init_method == 'non'){
    initial = initial_GMR_nonBinned(binnedY, K)
  } else {
    initial = initial_GMR(binnedY, K)
  }
  X = binnedY$X
  X_bin = binnedY$X_bin
  Y_bin = binnedY$Y_bin
  ...
  flex = initial$flex
  weight_init = initial$weight_init
  idx_init = initial$idx_init
  theta0_init = initial$theta0_init
  theta_init = initial$theta_init
  resi_init = initial$resi_init
...
```{r}
# initialization
init = initialization(binnedY, K, B, min_count_ratio, init_method = '', lambda_alpha, lambda_theta)
```
...
```{r}
par(mfrow = c(1, 2), pty = 's', cex = 0.7)
plot(c(0, 0), xlim = c(1, TT), ylim = range(unlist(init$resi_init)), col = 'white')
for (t in 1:TT){
  index = binnedY$time_indicator == t
  points(rep(t, sum(index)), init$resi_init[[1]][index])
}
plot(c(0, 0), xlim = c(1, TT), ylim = range(unlist(init$resi_init)), col = 'white')
for (t in 1:TT){
  index = binnedY$time_indicator == t
  points(rep(t, sum(index)), init$resi_init[[2]][index])
}
```
...
for (t in 1:TT){
  index = binnedY$time_indicator == t
  points(rep(t, sum(index)), init$resi_init[[1]][index], cex = init$weight_init[index,1]/mean(binnedY$Y_count))
}
plot(c(0, 0), xlim = c(1, TT), ylim = range(unlist(init$resi_init)), col = 'white')
for (t in 1:TT){
  index = binnedY$time_indicator == t
  points(rep(t, sum(index)), init$resi_init[[2]][index], cex = init$weight_init[index,2]/mean(binnedY$Y_count))
}
```
...
plot_data_and_model(X, Y, Z, init$theta0_init, init$theta_init)
```

They are initial $\theta_0$ and $\theta$

```{r}
print(init$theta0_init)
...
E_step = function(X_bin,
                  ...
                  K){
  Nb = length(Y_count)
  likeli = matrix(0, nrow = Nb, ncol = K)
  for (k in 1:K){
    likeli[,k] = LogConcDEAD::dlcd(resi[[k]], g[[k]])
  }
  
  clust = apply(likeli, 1, which.max)
  weight = matrix(0, nrow = Nb, ncol = K)
  idx = matrix(F, nrow = Nb, ncol = K)
  for (i in 1:Nb){
    weight[i, clust[i]] = Y_count[i]
    idx[i, clust[i]] = T
  }
...
```{r}
idx_old = init$idx_init
resi_old = init$resi_init
alpha_old = init$alpha_init
g_old = init$g_init

## E-step: update responsibilities
Estep = E_step(X_bin, Y_count, resi_old, alpha_old, g_old, K)
resp_new = Estep$resp
...
In the M-step, we update the estimates of $(\alpha,\theta,g)$: We have already seen the `Mstep_alpha()`, `Mstep_shift()`, and `Mstep_g(). We only need to define a function to update $\theta$ here.
...
#' @param iter_eta threshold for the iteration. If the increment of loglikelihood is smaller than iter_eta,
#' we terminate the iterations.
#' @return weight_new N_bin by K matrix. weight_new[i, k] represents final weight of ith residual for kth group
#' @return idx_new N_bin by K logical matrix. idx_new[i, k] represents whether ith point belongs to kth group or not. 
#' @return theta0_new length K list, with `theta0_new[[k]]` being the final estimate for the intercept coefficient of the regression for kth group
#' @return theta_new length K list, with `theta_new[[k]]` being the p-by-1 vector. 
#' Final estimates for coefficients of the regression for kth group
#' @return alpha_new (p+1)-by-K matrix. The coefficients for the final cluster probabilities.
#' @return resi_new length K list with `resi_new[[k]]` N_bin-by-d final residual matrix
#' @return g_new length K list, with `g_new[[k]]` being the final `mlelcd` for the kth group
...
#' @export
iteration = function(binnedY, initial, K, lambda_alpha, lambda_theta, max_iter = 30, iter_eta = 1e-6){
  X = binnedY$X
  Y_bin = binnedY$Y_bin
  Y_count = binnedY$Y_count
  X_bin = binnedY$X_bin
  ...
  return(list(weight_new = weight_new,
     idx_new = idx_new,
...
<<B-param>>
#' @param min_count_ratio min count ratio
#' @param init_method choice of the initialization method
<<lambda_alpha-param>>
<<lambda_theta-param>>
#' @param max_iter number of maximum iterations of EM to perform
#' @param iter_eta threshold for the iteration. If the increment of loglikelihood is smaller than iter_eta,
...
#' @return iter
#' @export
main = function(X, Y, K, B = 0, min_count_ratio = 0, init_method = '', lambda_alpha, lambda_theta, max_iter = 30, iter_eta = 1e-6){
  # binning
  binnedY = binningY(X, Y, B = B, min_count_ratio = 0)
  
  print('passed binning')
  
  # initial GMR
  initial = initialization(binnedY, K, B, min_count_ratio, init_method, lambda_alpha, lambda_theta)
  
  print('passed init')
  
  # iteration
  iter = iteration(binnedY, initial, K, lambda_alpha, lambda_theta, iter_eta, max_iter)
  
  return(list(binnedY = binnedY,
              initial = initial,
              iter = iter))
}
```
...
#' @return res
#' @export
mixLcdReg = function(X, Y, K, B = 0, min_count_ratio = 0, init_method = '', lambda_alpha, lambda_theta, max_iter = 30, iter_eta = 1e-6){
  # initial GMR
  if (init_method == 'th') {
    print('Initialization based on thresholded binned data')
    res = main(X, Y, K, B, min_count_ratio, init_method = 'th', lambda_alpha, lambda_theta, iter_eta, max_iter)
  } else if (init_method == 'non'){
    print('Initialization based on non-binned data')
    res = main(X, Y, K, B, 0, init_method = 'non', lambda_alpha, lambda_theta, iter_eta, max_iter)
  } else if (init_method == 'bin'){
    print('Initialization based on binned data')
    res = main(X, Y, K, B, 0, init_method = '', lambda_alpha, lambda_theta, iter_eta, max_iter)
  } else {
    print('No initialization method was chosen. Will be selected authomatically based on the performance')
    
    print('Initialization based on binned data')
    res1 = main(X, Y, K, B, min_count_ratio, init_method = '', lambda_alpha, lambda_theta, iter_eta, max_iter)
    res_loglik = c(res1$iter$Q[length(res1$iter$Q)])
    print(paste('The final loglikelihood with GMR initialization is', res_loglik[1]))
    
    print('Initialization based on thresholded binned data')
    res2 = main(X, Y, K, B, min_count_ratio, init_method = 'th', lambda_alpha, lambda_theta, iter_eta, max_iter)
    res_loglik = c(res_loglik, res2$iter$Q[length(res2$iter$Q)])
    print(paste('The final loglikelihood with GMR initialization on thresholded binned data is', res_loglik[2]))
    
    print('Initialization based on non-binned data')
    res3 = main(X, Y, K, B, min_count_ratio, init_method = 'non', lambda_alpha, lambda_theta, iter_eta, max_iter)
    res_loglik = c(res_loglik, res3$iter$Q[length(res3$iter$Q)])
    print(paste('The final loglikelihood with GMR initialization on non-binned data is', res_loglik[3]))
    
    if (which.max(res_loglik) == 2) {
      print('GMR initialization on thresholded binned data was chosen')
      return(res2)
    } else if (which.max(res_loglik) == 3) {
      print('GMR initialization on non-binned data was chosen')
      return(res3)
    } else {
      print('The default GMR initialization was chosen')
      return(res1)
    }
  }
  return(res)
...
#' @return diff
#' @export
L1_diff = function(true, est, K){
  diff = 0
  for (k in 1:K){
    diff = diff + sum(abs(true[[k]] - est[[k]]))
  }
  return(diff)
}
```
...
```{r}
set.seed(1)
mix1 = mixLcdReg(X, Y, B = 30, min_count_ratio = 0.02, init_method = 'th', K = 2, lambda_alpha = 1e-8, lambda_theta = 0)
```
...
plot(0:(length(mix1$iter$Q)-1), mix1$iter$Q, type ='b', main = 'The mixture of loglikelihood')
print(mix1$iter$Q)
```
...
```{r}
print(theta0_true)
print(mix1$init$theta0_init)
print(mix1$iter$theta0_new)
```
...
```{r}
print(theta_true)
print(mix1$init$theta_init)
print(mix1$iter$theta_new)
```
...
```{r}
print(alpha_true)
print(mix1$init$alpha_init)
print(mix1$iter$alpha_new)
```
...
```{r}
minI = min(unlist(mix1$iter$resi_new))
maxI = max(unlist(mix1$iter$resi_new))
grid0 = seq(minI, maxI, length = 200)

par(mfrow = c(1, 2), pty = 's', cex = 0.7)
plot(mix1$iter$g_new[[1]])
lines(grid0, exp(-grid0-1), lwd = 1, col = 'red')
plot(mix1$iter$g_new[[2]], xlab = 'residuals')
lines(grid0, exp(grid0-1), lwd = 1, col = 'red')
```
...
par(mfrow = c(1, 2), pty = 's', cex = 0.7)
plot_data_and_model(X, Y, Z, theta0_true, theta_true)
plot_data_and_model(X, Y, Z, mix1$iter$theta0_new, mix1$iter$theta_new)
```
...
```{r}
par(mfrow = c(1, 2), pty = 's', cex = 0.7)
plot_data_and_model(X, Y, Z, mix1$init$theta0_init, mix1$init$theta_init)
plot_data_and_model(X, Y, Z, mix1$iter$theta0_new, mix1$iter$theta_new)
```
